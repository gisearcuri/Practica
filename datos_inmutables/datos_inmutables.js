...
const juegoDivx3 = videojuegos.filter(juego => juego.id % 3 === 0);
//console.log(juegoDivx3);

//Un conjunto de videojuegos que pertenecen al género “acción-RPG”.
const juegosAventuraAccion = videojuegos.filter(juego => juego.genero === "acción-RPG");
//console.log(juegosAventuraAccion);

//Un conjunto de videojuegos que tienen más de un género.
const juegosGeneros = videojuegos.filter(juego => juego.genero.length >1 );
//console.log(juegosGeneros);

//Una lista con los nombres de los videojuegos.
const nombresVidJuegos = videojuegos.map(juego => juego.nombre);
//console.log(nombresVidJuegos)

//Una lista con los nombres de los videojuegos con un número de identificación superior a 19.
const juegosIdSup = videojuegos.filter(juego => juego.id >19) .map(juego => juego.nombre);
//console.log(juegosIdSup);
...
const genSegAventura = videojuegos.filter(juego => juego.genero.includes ("-aventura")).map(juego => juego.genero.slice(0,juego.genero.indexOf("-")));
//console.log(genSegAventura);

//Un conteo del número de videojuegos que son del género “party”.
const generoParty = videojuegos.filter (juego => juego.genero.includes ("party")).length;
//console.log(generoParty);

//Una lista con todos los videojuegos excepto aquellos cuyo número de identificación sea múltiplo de 5.
const juegoDivx5 = videojuegos.filter(juego => juego.id % 5 !== 0);
//console.log(juegoDivx5);

//Una lista con todos los videojuegos y para el videojuego con el número de identificación 5, se cambia su género por “otro”.
const generoOtro = [...videojuegos].map(juego => juego.id ==5? {...juego, genero: "otro"} : juego);
//console.log(generoOtro);